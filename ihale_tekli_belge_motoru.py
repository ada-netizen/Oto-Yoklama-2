...
def uret_yaklasik_maliyet_pdf(veri, hedef_klasor):
    ...
    elements.append(Paragraph("Y A K L A Ş I K   M A L İ Y E T   H E S A P   C E T V E L İ", style_title))
    elements.append(Spacer(1, 8*mm))
    ...
    header_data = [
        ["İdarenin Adı", ": Gazi Mustafa Kemal Anadolu Lisesi Müdürlüğü", "", Paragraph("İdarece Tespit\nve Takdir Edilen\nYaklaşık Maliyet\n(KDV Hariç)", style_center)],
        ["Yaklaşık Maliyeti Yapılan İş / Mal / Hizmetin Adı, Niteliği", f": {konu}", "", ""],
        ["Düzenleme Tarihi", f": {tarih}", "", Paragraph("Kişiler / Kurumlar / Firmalar ve Yaklaşık Maliyet İçin\nBildirilen Fiyatlar ( KDV Hariç )", style_center)]
    ]
    
    htbl = Table(header_data, colWidths=[70*mm, 100*mm, 20*mm, 50*mm])
    ...
    h1_raw = ["Sıra<br/>No", "Yaklaşık Maliyeti Hesaplanacak Mal ve Hizmetin", "", "", "", f"1<br/>{f1}", "", f"2<br/>{f2}", "", f"3<br/>{f3}", "", f"4<br/>{f4}", "", "Birim Yaklaşık<br/>Maliyet Fiyatı", "Toplam Yaklaşık<br/>Maliyet Fiyatı"]
    h1 = [Paragraph(x, style_center) if x else "" for x in h1_raw]
    
    h2_raw = ["", "Cinsi", "Özelliği", "Miktarı", "Ölçüsü", "Birim Fiyat", "Toplam Fiyat", "Birim Fiyat", "Toplam Fiyat", "Birim Fiyat", "Toplam Fiyat", "Birim Fiyat", "Toplam Fiyat", "", ""]
    h2 = [Paragraph(x, style_center) if x else "" for x in h2_raw]
    ...
    for idx, k in enumerate(gecerli_kalemler):
        ...
        row.extend([yak_maliyet_birim, yak_maliyet_toplam])
        table_data.append(row)
        
    t_row = [
        "", "Kişiler / Firmalarca Bildirilen Yaklaşık Maliyet Fiyatları Toplamı ( KDV HARİÇ )", "", "", "", 
        f"{col_sums[0]:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".") if col_sums[0]>0 else "TOPLAM", "", 
        f"{col_sums[1]:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".") if col_sums[1]>0 else "TOPLAM", "", 
        f"{col_sums[2]:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".") if col_sums[2]>0 else "TOPLAM", "", 
        f"{col_sums[3]:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".") if col_sums[3]>0 else "TOPLAM", "", 
        "", f"{col_sums[4]:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".") if col_sums[4]>0 else "TOPLAM"
    ]
    table_data.append(t_row)
        
    col_widths = [10*mm, 25*mm, 25*mm, 15*mm, 15*mm, 15*mm, 15*mm, 15*mm, 15*mm, 15*mm, 15*mm, 15*mm, 15*mm, 20*mm, 20*mm]
    t = Table(table_data, colWidths=col_widths)
    ...
    t_style = [
        ...
        ('SPAN', (1, len(table_data)-1), (4, len(table_data)-1)),
        ('ALIGN', (1, len(table_data)-1), (4, len(table_data)-1), 'RIGHT'),
        ('SPAN', (5, len(table_data)-1), (6, len(table_data)-1)),
        ('SPAN', (7, len(table_data)-1), (8, len(table_data)-1)),
        ('SPAN', (9, len(table_data)-1), (10, len(table_data)-1)),
    ...
    if len(komisyon_uyeleri) > 0:
        ...
        col_w = 240*mm / len(komisyon_uyeleri)
        sig_table = Table(imza_data, colWidths=[col_w] * len(komisyon_uyeleri))
        sig_table.setStyle(TableStyle([
            ('ALIGN', (0,0), (-1,-1), 'CENTER'),
            ('VALIGN', (0,0), (-1,-1), 'TOP'),
...
def uret_yaklasik_maliyet_excel(veri, hedef_klasor):
    ...
    c1 = ws.cell(row=1, column=1, value="Y A K L A Ş I K   M A L İ Y E T   H E S A P   C E T V E L İ")
    c1.font = bold_title
    ...
    ws.cell(row=3, column=2, value=": Gazi Mustafa Kemal Anadolu Lisesi Müdürlüğü").font = normal_font
    ws.merge_cells('B3:E3')
    ws.cell(row=3, column=14, value="İdarece Tespit\nve Takdir Edilen\nYaklaşık Maliyet\n(KDV Hariç)").font = normal_font
    ws.cell(row=3, column=14).alignment = center_align
    ws.merge_cells('N3:O5')
    
    ws.cell(row=4, column=1, value="Yaklaşık Maliyeti Yapılan İş / Mal / Hizmetin Adı, Niteliği").font = normal_font
    ws.cell(row=4, column=2, value=f": {konu}").font = normal_font
    ws.merge_cells('B4:E4')
    
    ws.cell(row=5, column=1, value="Düzenleme Tarihi").font = normal_font
    ws.cell(row=5, column=2, value=f": {tarih}").font = normal_font
    ws.merge_cells('B5:E5')
    
    ws.merge_cells('F5:M5')
    ws.cell(row=5, column=6, value="Kişiler / Kurumlar / Firmalar ve Yaklaşık Maliyet İçin\nBildirilen Fiyatlar ( KDV Hariç )").font = normal_font
    ...
    h1 = ["Sıra\nNo", "Yaklaşık Maliyeti Hesaplanacak Mal ve Hizmetin", "", "", "", f"1\n{f1}", "", f"2\n{f2}", "", f"3\n{f3}", "", f"4\n{f4}", "", "Birim Yaklaşık\nMaliyet Fiyatı", "Toplam Yaklaşık\nMaliyet Fiyatı"]
    ...
    r += 1
    h2 = ["", "Cinsi", "Özelliği", "Miktarı", "Ölçüsü", "Birim Fiyat", "Toplam Fiyat", "Birim Fiyat", "Toplam Fiyat", "Birim Fiyat", "Toplam Fiyat", "Birim Fiyat", "Toplam Fiyat", "", ""]
    ...
    r += 4
    ws.cell(row=r, column=1, value="Not:Yaklaşık Maliyet Hesap Cetveli ve Ekleri İhale Onay Belgesi'ne Eklenecektir.").font = normal_font
